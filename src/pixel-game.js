...
// Loading Scene - Animated pixelated loading screen
class LoadingScene extends Phaser.Scene {
    ...
    hideAllUI() {
        const uiElements = [
            ...
            'inventory-button', 'email-button', 'phone-button',
            'leaderboard-button', 'interaction-prompt', 'notification'
        ];
        uiElements.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.style.display = 'none';
...
class CharacterCustomizationScene extends Phaser.Scene {
        ...
        try {
            ...
            this.time.delayedCall(100, () => {
                if (this.textures.exists(initialPreviewKey)) {
                    // Now safe to create sprite
                    this.characterPreview = this.add.sprite(previewX, previewY, initialPreviewKey);
                    this.characterPreview.setScale(8);
                    this.characterPreview.setDepth(10);
                } else {
                    // Fallback to default player texture
                    console.warn('Initial preview texture not ready, using default');
            ...
            console.error('Failed to create initial preview:', error);
            // Fallback: use default texture
            this.characterPreview = this.add.sprite(previewX, previewY, 'player');
            this.characterPreview.setScale(8);
            this.characterPreview.setDepth(10);
        }
    ...
    updateCharacterPreview() {
        ...
        try {
                ...
                try {
                    if (this.textures.exists(previewKey) && this.characterPreview && this.characterPreview.active) {
                        ...
                        this.tweens.add({
                            ...
                            ease: 'Sine.easeInOut',
                            onComplete: () => {
                                // Clean up old textures to prevent memory leak
                                this.textures.each((texture) => {
                                    if (texture.key.startsWith('player_preview_') && texture.key !== previewKey) {
                                        try {
            ...
            console.error('Failed to create custom sprite:', error);
            this.isUpdatingPreview = false;
        }
    }
...
function initGame() {
    try {
        ...
        if (loading) {
            loading.innerHTML = '<div style="color: red;">Game initialization failed. Check console for errors.</div>';
        }
    }
}
...
function createGame() {
    try {
        ...
        const loading = document.getElementById('loading');
        if (loading) {
            loading.innerHTML = '<div style="color: red;">Failed to load game.<br>Please check browser console for details.</div>';
        }
    }
}
...
// CRITICAL: Hide story overlay on page load
